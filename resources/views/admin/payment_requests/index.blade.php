    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
            <div class="flex items-center gap-2 mb-3">
                ...
                <span class="text-[10px] font-black orbitron text-purple-500 uppercase tracking-[0.3em]">ADMINSTRATIVE GATEWAY</span>
            </div>
            <h1 class="text-4xl font-black orbitron italic uppercase tracking-tighter text-white">
                PAYMENT <span class="text-purple-500 text-glow">REQUESTS</span>
            </h1>
            <p class="text-gray-500 text-xs font-bold mt-2 uppercase tracking-[0.2em]">Manual Verification of Subscription Upgrades</p>
        </div>
    </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-white/[0.02] border-b border-white/5">
                        <th class="px-10 py-8 text-[10px] font-black orbitron text-gray-400 uppercase tracking-[0.3em]">User</th>
                        <th class="px-10 py-8 text-[10px] font-black orbitron text-gray-400 uppercase tracking-[0.3em]">Plan</th>
                        <th class="px-10 py-8 text-[10px] font-black orbitron text-gray-400 uppercase tracking-[0.3em]">Amount</th>
                        <th class="px-10 py-8 text-[10px] font-black orbitron text-gray-400 uppercase tracking-[0.3em]">UTR Number</th>
                        <th class="px-10 py-8 text-[10px] font-black orbitron text-gray-400 uppercase tracking-[0.3em]">Screenshot</th>
                        <th class="px-10 py-8 text-[10px] font-black orbitron text-gray-400 uppercase tracking-[0.3em]">Status</th>
                        <th class="px-10 py-8 text-right text-[10px] font-black orbitron text-gray-400 uppercase tracking-[0.3em]">Actions</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-white/5">
                    ...
                    <tr class="group hover:bg-white/[0.01] transition-all duration-500">
                        <td class="px-10 py-8">
                            <span class="text-sm font-black orbitron text-white uppercase tracking-tight">{{ $request->user->username ?? 'N/A' }}</span>
                        </td>
                        <td class="px-10 py-8">
                        ...
                        <td class="px-10 py-8">
                            <span class="text-sm font-black orbitron text-purple-400">₹{{ number_format($request->amount, 0) }}</span>
                        </td>
                        <td class="px-10 py-8">
                        ...
                        <td class="px-10 py-8">
                            ...
                            <button onclick="viewProof('{{ asset($request->payment_screenshot) }}')" class="w-10 h-10 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 flex items-center justify-center hover:bg-emerald-500/20 transition-all group/btn">
                                <i data-lucide="image" class="w-4 h-4"></i>
                            </button>
                            @else
                            <span class="text-[8px] text-gray-600 font-black orbitron">NO_PROOF</span>
                            @endif
                        </td>
                        <td class="px-10 py-8">
                            <span class="px-4 py-1.5 rounded-full text-[9px] font-black orbitron uppercase tracking-[0.2em] 
                        ...
                        <td class="px-10 py-8 text-right">
                            <div class="flex justify-end gap-3">
                                ...
                                <form action="{{ route('admin.payments.upgrade.approve', $request->id) }}" method="POST">
                                    @csrf
                                    <button type="submit" onclick="return confirm('Approve this upgrade?')" class="px-6 py-2.5 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-black text-[9px] orbitron uppercase tracking-widest transition-all">
                                        Approve
                                    </button>
                                </form>
                                ...
                                @else
                                <span class="text-[8px] font-black orbitron text-gray-800 uppercase tracking-[0.4em] italic">PROCESSED</span>
                                @endif
                            </div>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="7" class="px-10 py-32 text-center opacity-20 italic orbitron text-[10px] tracking-widest">
...
<div id="proof-modal" class="fixed inset-0 bg-black/95 backdrop-blur-3xl z-[200] hidden items-center justify-center p-4">
    <div class="relative max-w-5xl w-full bg-[#0c0518] border border-white/10 rounded-[3rem] overflow-hidden shadow-2xl">
        <div class="flex items-center justify-between p-8 border-b border-white/5">
            <span class="text-[10px] font-black orbitron text-white uppercase tracking-[0.3em]">PAYMENT PROOF</span>
...
<div id="reject-modal" class="fixed inset-0 bg-black/95 backdrop-blur-3xl z-[200] hidden items-center justify-center p-6">
    <div class="max-w-xl w-full glass-card border border-white/10 rounded-[2.5rem] p-12 space-y-10 shadow-2xl">
        <div class="flex items-center justify-between">
            <h3 class="orbitron text-xl font-black text-rose-500 uppercase tracking-tighter italic">REJECT <span class="text-white">REQUEST</span></h3>
        ...
        <form id="reject-form" method="POST" class="space-y-8">
            @csrf
            <div class="space-y-4">
                <label class="block text-[10px] font-black orbitron text-gray-600 uppercase tracking-widest">Reason for Rejection</label>
            ...
            <button type="submit" class="w-full py-5 bg-rose-600 text-white font-black orbitron uppercase text-[10px] rounded-2xl">Reject Request</button>
        </form>
    </div>
</div>
