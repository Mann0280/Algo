...
@section('title', 'NEURAL PACKAGE PROTOCOLS')
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
            <div class="flex items-center gap-2 mb-3">
                ...
                <span class="text-[10px] font-black orbitron text-purple-500 uppercase tracking-[0.3em]">SUBSCRIPTION INFRASTRUCTURE</span>
            </div>
            ...
            <p class="text-gray-500 text-xs font-bold mt-2 uppercase tracking-[0.2em]">Manage Tiered Access and Neural Entitlements</p>
        </div>
        ...
        <a href="{{ route('admin.premium-packages.create') }}" class="group relative px-8 py-4 bg-purple-600 rounded-2xl overflow-hidden shadow-[0_0_30px_rgba(147,51,234,0.3)]">
            ...
            <div class="relative flex items-center gap-3">
                ...
                <span class="text-[10px] font-black orbitron text-white uppercase tracking-[0.2em]">Initialize New Protocol</span>
            </div>
        </a>
    </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-white/[0.02] border-b border-white/5">
                        <th class="px-10 py-8 text-[10px] font-black orbitron text-gray-400 uppercase tracking-[0.3em]">Designation</th>
                        <th class="px-10 py-8 text-[10px] font-black orbitron text-gray-400 uppercase tracking-[0.3em]">Exchange Value</th>
                        <th class="px-10 py-8 text-[10px] font-black orbitron text-gray-400 uppercase tracking-[0.3em]">Temporal Scope</th>
                ...
                <tbody class="divide-y divide-white/5">
                    ...
                    <tr class="group hover:bg-white/[0.01] transition-all duration-500">
                        <td class="px-10 py-8">
                            <div class="flex items-center gap-5">
                                ...
                                <div class="flex flex-col">
                                    <span class="text-sm font-black orbitron text-white italic group-hover:text-purple-400 transition-colors uppercase tracking-tight">{{ $package->name }}</span>
                        ...
                        <td class="px-10 py-8">
                            <div class="flex flex-col">
                                <span class="text-lg font-black orbitron text-white italic">₹{{ number_format($package->price, 0) }}</span>
                                <span class="text-[9px] font-bold text-gray-700 uppercase tracking-widest mt-0.5">Base Transfer</span>
                            </div>
                        </td>
                        <td class="px-10 py-8">
                            <div class="flex items-center gap-2.5">
                                <i data-lucide="clock" class="w-3.5 h-3.5 text-gray-700"></i>
                                <span class="text-[10px] font-black orbitron text-gray-400 uppercase tracking-widest">{{ $package->duration_days }} Sidereal Days</span>
                            </div>
                        </td>
                        <td class="px-10 py-8">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-[10px] font-black orbitron {{ $package->is_active ? 'text-emerald-400' : 'text-rose-400' }} uppercase tracking-widest">
                                    {{ $package->is_active ? 'Online' : 'Decommissioned' }}
                                </span>
                            </div>
                        </td>
                        <td class="px-10 py-8">
                            <div class="flex justify-end gap-3">
                                <a href="{{ route('admin.premium-packages.edit', $package) }}" class="flex items-center justify-center w-10 h-10 rounded-xl bg-white/5 border border-white/10 text-gray-500 hover:text-white hover:bg-purple-600/20 hover:border-purple-500/30 transition-all group/btn">
                                ...
                                <form action="{{ route('admin.premium-packages.destroy', $package) }}" method="POST" onsubmit="return confirm('Deconstruct this package protocol?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="flex items-center justify-center w-10 h-10 rounded-xl bg-white/5 border border-white/10 text-gray-500 hover:text-rose-500 hover:bg-rose-500/10 hover:border-rose-500/20 transition-all group/btn">
                    ...
                    <tr>
                        <td colspan="5" class="px-10 py-32 text-center">
                            <div class="flex flex-col items-center gap-6 opacity-20">
                                ...
                                <span class="text-[10px] font-black orbitron uppercase tracking-[0.4em] text-white">Zero Protocol Signatures Detected</span>
                            </div>
                        </td>
                    </tr>
                    @endforelse
