...
<div class="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
    ...
    <div class="glass-panel rounded-[2.5rem] border-white/[0.05] overflow-hidden">
        <div class="p-8 border-b border-white/[0.05] bg-white/[0.01] flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <h3 class="text-sm font-bold text-gray-400 uppercase tracking-widest">Transaction Records</h3>
            </div>
                    ...
                    <tr class="hover:bg-white/[0.02] transition-colors group">
                        ...
                        <td class="px-8 py-6">
                            <div class="flex items-center gap-3">
                                ...
                                <span class="text-sm font-bold text-white group-hover:text-indigo-400 transition-colors uppercase">{{ $record->plan_name }}</span>
                            </div>
                        </td>
                        <td class="px-8 py-6">
                             <span class="text-sm font-bold text-white">₹{{ number_format($record->amount, 0) }}</span>
                        </td>
                        <td class="px-8 py-6">
                             <span class="text-xs font-bold {{ $record->expires_at && $record->expires_at->isPast() ? 'text-gray-600' : 'text-emerald-500' }}">
                                {{ $record->expires_at ? $record->expires_at->format('d M Y') : 'UNLIMITED' }}
                            </span>
                        </td>
                        <td class="px-8 py-6">
                             <span class="px-4 py-1.5 rounded-full text-[9px] font-bold uppercase tracking-widest {{ $record->status === 'Completed' ? 'bg-emerald-500/10 border border-emerald-500/20 text-emerald-500' : 'bg-rose-500/10 border border-rose-500/20 text-rose-500' }}">
                                {{ $record->status }}
                            </span>
                        </td>
                        <td class="px-8 py-6">
                            <a href="{{ route('account.receipt', $record->id) }}" class="p-2.5 rounded-xl bg-white/[0.05] border border-white/[0.05] text-gray-500 hover:text-white hover:bg-purple-600/20 hover:border-purple-500/30 transition-all inline-block">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="glass-panel p-10 rounded-[2.5rem] border-white/[0.05] flex flex-col justify-center items-center text-center group">
            <p class="text-[10px] font-bold text-gray-600 uppercase tracking-widest">Total Investment</p>
            <p class="text-4xl font-bold text-white mt-1 group-hover:text-purple-500 transition-colors">
...
<!-- Digital Receipt Modal Removed -->

@push('scripts')
<script>
    // Receipt modal removed as per user request for direct access.
</script>
@endpush
@endsection
