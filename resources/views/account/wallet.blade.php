...
<div class="space-y-10">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
            <h2 class="text-3xl font-black text-white orbitron italic tracking-tighter">NEURAL <span class="text-purple-500">WALLET</span></h2>
            <p class="text-sm text-gray-500 mt-1 uppercase tracking-widest font-bold">Financial Protocol Dashboard</p>
        </div>
        <div class="flex items-center gap-4">
            <a href="{{ route('account.withdraw') }}" class="px-8 py-3.5 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-2xl text-[10px] font-black orbitron uppercase tracking-[0.2em] hover:scale-105 hover:shadow-[0_0_30px_rgba(147,51,234,0.3)] transition-all">
                Request Withdrawal
            </a>
        </div>
    </div>
    ...
    <div class="glass-panel rounded-[2.5rem] p-8 border-white/[0.05] relative overflow-hidden group">
        ...
        <div class="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            <div>
                <label class="text-[10px] font-black orbitron text-gray-500 uppercase tracking-[0.3em]">Total Wallet Balance</label>
                <div class="flex items-baseline gap-2 mt-2">
                    <span class="text-2xl font-bold text-purple-500">₹</span>
                    <h3 class="text-6xl font-black text-white orbitron tracking-tighter tabular-nums">{{ number_format($user->wallet_balance, 2) }}</h3>
                </div>
            </div>
            <div class="flex items-center gap-2 px-4 py-2 bg-emerald-500/5 border border-emerald-500/10 rounded-xl h-fit">
                <div class="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></div>
                <span class="text-[10px] font-bold text-emerald-500 orbitron">SECURE ASSETS</span>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="glass-panel rounded-[2rem] p-6 border-white/[0.05] flex flex-col justify-between group hover:border-purple-500/30 transition-all duration-500">
            ...
            <div class="mt-4">
                <p class="text-[8px] font-black orbitron text-gray-500 uppercase tracking-widest">Total Earned</p>
        ...
        <div class="glass-panel rounded-[2rem] p-6 border-white/[0.05] flex flex-col justify-between group hover:border-indigo-500/30 transition-all duration-500">
            ...
            <div class="mt-4">
                <p class="text-[8px] font-black orbitron text-gray-500 uppercase tracking-widest">Withdrawn</p>
        ...
        <div class="glass-panel rounded-[2rem] p-6 border-white/[0.05] flex flex-col justify-between group hover:border-emerald-500/30 transition-all duration-500">
            ...
            <div class="mt-4">
                <p class="text-[8px] font-black orbitron text-gray-500 uppercase tracking-widest">Available</p>
        ...
        <div class="glass-panel rounded-[2rem] p-6 border-white/[0.05] flex flex-col justify-between group hover:border-amber-500/30 transition-all duration-500">
            ...
            <div class="mt-4">
                <p class="text-[8px] font-black orbitron text-gray-500 uppercase tracking-widest">Pending</p>
    ...
    <div class="glass-panel rounded-[2.5rem] border-white/[0.05] overflow-hidden">
        <div class="px-8 py-6 border-b border-white/[0.05] flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <h3 class="text-lg font-bold text-white tracking-wide uppercase text-[12px] orbitron">Transaction Records</h3>
            </div>
        </div>
            ...
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-white/[0.03]">
                    ...
                    <tr class="group hover:bg-white/[0.02] transition-colors">
                        ...
                        <td class="px-8 py-5">
                            @if($tx->type === 'credit')
                                <span class="flex items-center gap-2 text-emerald-500 text-xs font-bold uppercase orbitron italic tracking-widest">
                                    <i data-lucide="arrow-down-left" class="w-3.5 h-3.5"></i> Credit
                                </span>
                            @elseif($tx->type === 'debit')
                                <span class="flex items-center gap-2 text-rose-500 text-xs font-bold uppercase orbitron italic tracking-widest">
                                    <i data-lucide="arrow-up-right" class="w-3.5 h-3.5"></i> Debit
                                </span>
                            @else
                                <span class="flex items-center gap-2 text-amber-500 text-xs font-bold uppercase orbitron italic tracking-widest">
                                    <i data-lucide="external-link" class="w-3.5 h-3.5"></i> Withdraw
                                </span>
                            @endif
                        </td>
                        <td class="px-8 py-5">
                            <span class="text-sm font-black orbitron tabular-nums {{ $tx->type === 'credit' ? 'text-emerald-500' : 'text-rose-500' }}">
                        ...
                        <td class="px-8 py-5 text-right">
                            <span class="px-3 py-1 rounded-lg text-[9px] font-black orbitron uppercase tracking-widest 
                    ...
                    <tr>
                        <td colspan="5" class="px-8 py-20 text-center">
                            <div class="flex flex-col items-center gap-4">
                                ...
                                <p class="text-sm font-bold text-gray-600 orbitron uppercase tracking-[0.2em]">No financial activity logged yet</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
