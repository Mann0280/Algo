...
<div class="space-y-10">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
            <h2 class="text-3xl font-bold text-white tracking-tighter uppercase">REFER & <span class="text-purple-500">EARN</span></h2>
        ...
        <div class="flex items-center gap-3 px-6 py-3 bg-white/[0.02] border border-white/[0.05] rounded-2xl">
            <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Your Code:</span>
            <span class="text-lg font-bold text-white tracking-widest">{{ $user->referral_code }}</span>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 glass-panel rounded-[2.5rem] p-8 sm:p-10 border-white/[0.05] relative overflow-hidden group flex flex-col justify-center">
            ...
            <div class="relative z-10">
                ...
                <div class="p-2 bg-black/40 rounded-3xl border border-white/10 flex flex-col sm:flex-row items-center gap-4 transition-all focus-within:border-purple-500/50 shadow-2xl">
                    ...
                    <button id="copy-link-btn" class="w-full sm:w-auto px-10 py-4 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-2xl text-[10px] font-bold uppercase tracking-[0.2em] transition-all hover:scale-[1.03] shadow-lg">
                        Copy Link
                    </button>
                </div>
                <div class="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-6">
                    <div class="flex flex-col">
                        <span class="text-[9px] font-bold text-gray-500 uppercase tracking-widest">Plan Rewards</span>
                        <span class="text-xs font-bold text-emerald-400 mt-1">Up to ₹5000 / Invite</span>
                    </div>
                    <div class="flex flex-col">
                        <span class="text-[9px] font-bold text-gray-500 uppercase tracking-widest">Payouts</span>
                        <span class="text-xs font-bold text-indigo-400 mt-1">Direct to Wallet</span>
                    </div>
                    <div class="flex flex-col">
                        <span class="text-[9px] font-bold text-gray-500 uppercase tracking-widest">Limit</span>
        ...
        <div class="flex flex-col gap-6">
            <div class="flex-1 glass-panel rounded-[2rem] p-8 border-white/[0.05] flex flex-col justify-between">
                <div>
                    <label class="text-[10px] font-bold text-gray-500 uppercase tracking-[0.2em]">Total Refers</label>
                    <p class="text-4xl font-bold text-white mt-1">{{ number_format($stats['total_referrals']) }}</p>
                </div>
            ...
            <div class="flex-1 glass-panel rounded-[2rem] p-8 border-white/[0.05] flex flex-col justify-between">
                <div>
                    <label class="text-[10px] font-bold text-gray-500 uppercase tracking-[0.2em]">Total Earnings</label>
                    <p class="text-4xl font-bold text-purple-500 mt-1">₹{{ number_format($stats['total_earnings'], 2) }}</p>
                </div>
                <div class="mt-6 flex items-center justify-between text-[11px] font-bold">
    ...
    <div class="glass-panel rounded-[2.5rem] border-white/[0.05] overflow-hidden">
        <div class="px-8 py-6 border-b border-white/[0.05] flex items-center justify-between">
            <h3 class="text-lg font-bold text-white tracking-wide uppercase text-[12px] flex items-center gap-3">
        ...
        <div class="overflow-x-auto no-scrollbar">
            <table class="w-full text-left">
                <thead>
                    <tr class="text-[9px] font-bold text-gray-500 uppercase tracking-[0.2em] bg-white/[0.01]">
                    ...
                    <tr class="group hover:bg-white/[0.02] transition-colors">
                        <td class="px-8 py-5">
                            <div class="flex items-center gap-4">
                                <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-600/20 to-purple-600/20 flex items-center justify-center text-xs font-bold text-indigo-400">
                        ...
                        <td class="px-8 py-5">
                            @if($ref->status === 'rewarded')
                                <span class="px-3 py-1 rounded-lg bg-emerald-500/10 text-emerald-500 text-[9px] font-bold uppercase tracking-widest border border-emerald-500/20">Success</span>
                            @elseif($ref->status === 'pending')
                                <span class="px-3 py-1 rounded-lg bg-amber-500/10 text-amber-500 text-[9px] font-bold uppercase tracking-widest border border-amber-500/20">Pending Purchase</span>
                            @else
                                <span class="px-3 py-1 rounded-lg bg-rose-500/10 text-rose-500 text-[9px] font-bold uppercase tracking-widest border border-rose-500/20">Rejected</span>
                            @endif
                        </td>
                        ...
                        <td class="px-8 py-5 text-right">
                            <span class="text-sm font-bold tabular-nums {{ $ref->reward_amount > 0 ? 'text-emerald-500' : 'text-gray-600' }}">
                    ...
                    <tr>
                        <td colspan="5" class="px-8 py-20 text-center">
                            <div class="flex flex-col items-center gap-4">
                                ...
                                <p class="text-sm font-bold text-gray-600 uppercase tracking-[0.2em]">Zero network connections detected</p>
