...
<div class="max-w-4xl mx-auto space-y-10">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
            <h2 class="text-3xl font-black text-white orbitron italic tracking-tighter">WITHDRAW <span class="text-purple-500">FUNDS</span></h2>
            <p class="text-sm text-gray-500 mt-1 uppercase tracking-widest font-bold">Secure Payout Protocol</p>
        </div>
        <a href="{{ route('account.wallet') }}" class="flex items-center gap-2 text-xs font-bold text-gray-400 hover:text-white transition-colors uppercase orbitron tracking-widest">
    ...
    <div class="glass-panel rounded-[2rem] p-8 border-white/[0.05] relative overflow-hidden group bg-gradient-to-br from-purple-600/[0.02] to-transparent">
        <div class="flex items-center justify-between relative z-10">
            <div>
                <label class="text-[9px] font-black orbitron text-gray-500 uppercase tracking-[0.3em]">Available for Withdrawal</label>
                <div class="flex items-baseline gap-2 mt-2">
                    <span class="text-xl font-bold text-purple-500">₹</span>
                    <h3 class="text-4xl font-black text-white orbitron tracking-tighter tabular-nums">{{ number_format($availableBalance, 2) }}</h3>
                </div>
            </div>
            <div class="hidden sm:block">
                <div class="px-6 py-3 bg-emerald-500/5 border border-emerald-500/10 rounded-2xl">
                    <div class="flex items-center gap-3">
                        ...
                        <span class="text-[10px] font-bold text-emerald-400 orbitron">VERIFIED ASSETS</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <form action="{{ route('account.withdraw.store') }}" method="POST" class="glass-panel rounded-[2.5rem] p-8 sm:p-10 border-white/[0.05] space-y-8">
                ...
                @if ($errors->any())
                    <div class="p-4 bg-rose-500/10 border border-rose-500/20 rounded-2xl">
                        <ul class="list-disc list-inside text-xs text-rose-500 font-bold orbitron uppercase tracking-widest space-y-1">
                ...
                <div class="space-y-6">
                    <!-- Amount -->
                    <div class="space-y-3">
                        <label class="text-[11px] font-bold text-gray-400 orbitron uppercase tracking-widest ml-2">Withdrawal Amount (₹)</label>
                        <div class="relative group/field">
                            ...
                            <input type="number" name="amount" min="500" max="{{ $availableBalance }}" step="0.01" required 
                                   class="w-full bg-white/[0.03] border border-white/10 rounded-2xl pl-14 pr-6 py-4 text-white text-lg font-bold orbitron focus:outline-none focus:border-purple-500/50 focus:bg-white/[0.06] transition-all shadow-inner" 
                                   placeholder="Min. 500">
                        </div>
                        <p class="text-[10px] text-gray-600 font-bold uppercase tracking-widest ml-2">Minimum Withdrawal amount is ₹500</p>
                    </div>

                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <!-- Bank Name -->
                        <div class="space-y-3">
                            <label class="text-[11px] font-bold text-gray-400 orbitron uppercase tracking-widest ml-2">Bank Name</label>
                        ...
                        <!-- IFSC Code -->
                        <div class="space-y-3">
                            <label class="text-[11px] font-bold text-gray-400 orbitron uppercase tracking-widest ml-2">IFSC Code</label>
                    ...
                    <!-- Account Number -->
                    <div class="space-y-3">
                        <label class="text-[11px] font-bold text-gray-400 orbitron uppercase tracking-widest ml-2">Account Number</label>
                    ...
                    <!-- UPI ID -->
                    <div class="space-y-3">
                        <label class="text-[11px] font-bold text-gray-400 orbitron uppercase tracking-widest ml-2">UPI ID (Optional)</label>
                ...
                <div class="pt-4">
                    <button type="submit" class="w-full py-5 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-2xl text-[12px] font-black orbitron uppercase tracking-[0.3em] hover:scale-[1.02] hover:shadow-[0_0_40px_rgba(147,51,234,0.4)] transition-all">
                        Initiate Withdrawal
                    </button>
        ...
        <div class="space-y-6">
            <div class="glass-panel rounded-[2rem] p-8 border-white/[0.05] space-y-6">
                <h4 class="text-xs font-black orbitron text-white uppercase tracking-widest">Withdrawal Protocol</h4>
                <div class="space-y-5">
            ...
            <div class="p-8 rounded-[2rem] bg-gradient-to-br from-indigo-600/10 to-transparent border border-white/[0.05]">
                <p class="text-[10px] font-bold text-indigo-400 orbitron uppercase tracking-widest leading-relaxed">
