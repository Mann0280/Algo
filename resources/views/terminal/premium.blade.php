...
<div class="container mx-auto px-4 max-w-[1400px] pb-20">
    ...
    <div class="flex items-center justify-between mb-5">
        <div class="flex items-center gap-4">
            <h1 class="text-2xl font-black orbitron text-white uppercase italic tracking-tight">Trading <span class="text-purple-500">Terminal</span></h1>
            <div class="flex items-center gap-2 px-3 py-1 bg-emerald-500/10 border border-emerald-500/20 rounded-full">
                ...
                <span class="text-[9px] font-bold text-emerald-500 orbitron uppercase">Market Open</span>
            </div>
        </div>
        <div class="flex items-center gap-3">
            <div class="text-right hidden sm:block">
                <div class="text-[9px] text-gray-500 orbitron font-bold uppercase">Last Sync</div>
    ...
    <div class="bg-white/[0.02] border border-white/5 rounded-xl mb-5 px-4 overflow-hidden">
        <div class="ticker-strip" id="tickerStrip">
            ...
            <div class="ticker-item orbitron"><span class="text-gray-400">{{ $t['n'] }}</span><span class="text-white font-black">{{ $t['p'] }}</span><span class="{{ $t['u'] ? 'text-emerald-400' : 'text-rose-400' }}">{{ $t['c'] }}</span></div>
            @endforeach @endfor
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
        ...
        <div class="widget flex items-center gap-3">
            ...
            <div><div class="text-[8px] font-bold text-gray-500 uppercase orbitron">{{ $s['l'] }}</div><div class="text-lg font-black text-white orbitron leading-none">{{ $s['v'] }}</div></div>
        </div>
        @endforeach
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
        {{-- Sector Heatmap --}}
        <div class="widget">
            <div class="widget-title orbitron flex items-center gap-2"><i data-lucide="layout-grid" class="w-3.5 h-3.5 text-purple-400"></i> Sector Performance</div>
            <div class="grid grid-cols-2 gap-2">
                @foreach($sectors as $sec)
                <div class="sector-pill">
                    <span class="text-xs font-bold text-gray-300">{{ $sec['name'] }}</span>
                    <span class="text-xs font-black orbitron {{ $sec['up'] ? 'text-emerald-400' : 'text-rose-400' }}">{{ $sec['change'] }}</span>
                </div>
                @endforeach
            </div>
        </div>
        ...
        {{-- Market Breadth --}}
        <div class="widget">
            <div class="widget-title orbitron flex items-center gap-2"><i data-lucide="bar-chart-3" class="w-3.5 h-3.5 text-purple-400"></i> Market Breadth</div>
            <div class="flex items-center gap-2 mb-3">
                <div class="flex-1 text-center"><div class="text-[8px] font-bold text-gray-500 uppercase">Advances</div><div class="text-lg font-black text-emerald-400 orbitron">{{ number_format($market_stats['advances']) }}</div></div>
                <div class="w-[1px] h-8 bg-white/5"></div>
                <div class="flex-1 text-center"><div class="text-[8px] font-bold text-gray-500 uppercase">Declines</div><div class="text-lg font-black text-rose-400 orbitron">{{ number_format($market_stats['declines']) }}</div></div>
                <div class="w-[1px] h-8 bg-white/5"></div>
                <div class="flex-1 text-center"><div class="text-[8px] font-bold text-gray-500 uppercase">Unchanged</div><div class="text-lg font-black text-gray-400 orbitron">{{ $market_stats['unchanged'] }}</div></div>
            </div>
            ...
            <div class="grid grid-cols-3 gap-2 mt-4">
                <div class="text-center p-2 bg-white/[0.02] rounded-lg"><div class="text-[7px] font-bold text-gray-500 uppercase">Volume</div><div class="text-[11px] font-black text-white orbitron">{{ $market_stats['total_volume'] }}</div></div>
                <div class="text-center p-2 bg-emerald-500/5 rounded-lg"><div class="text-[7px] font-bold text-emerald-500/60 uppercase">FII Flow</div><div class="text-[11px] font-black text-emerald-400 orbitron">{{ $market_stats['fii_flow'] }}</div></div>
                <div class="text-center p-2 bg-blue-500/5 rounded-lg"><div class="text-[7px] font-bold text-blue-500/60 uppercase">DII Flow</div><div class="text-[11px] font-black text-blue-400 orbitron">{{ $market_stats['dii_flow'] }}</div></div>
            </div>
        </div>
        ...
        <div class="widget">
            <div class="flex items-center gap-2 mb-3">
                <button class="tab-btn active orbitron" id="tabGainers" onclick="showMovers('gainers')">🟢 Gainers</button>
                <button class="tab-btn orbitron" id="tabLosers" onclick="showMovers('losers')">🔴 Losers</button>
            </div>
            <div id="moverGainers">
                ...
                <div class="flex items-center justify-between py-1.5 border-b border-white/[0.02]">
                    ...
                    <div class="flex items-center gap-3"><span class="text-xs font-bold text-white">{{ $g['price'] }}</span><span class="text-xs font-black text-emerald-400 orbitron">{{ $g['change'] }}</span></div>
                </div>
                @endforeach
            </div>
            <div id="moverLosers" style="display:none;">
                ...
                <div class="flex items-center justify-between py-1.5 border-b border-white/[0.02]">
                    ...
                    <div class="flex items-center gap-3"><span class="text-xs font-bold text-white">{{ $l['price'] }}</span><span class="text-xs font-black text-rose-400 orbitron">{{ $l['change'] }}</span></div>
                </div>
                @endforeach
            </div>
        </div>
    ...
    <div class="terminal-grid">
        ...
        <div class="space-y-4">
            <div class="flex items-center justify-between mb-2">
                <h2 class="text-sm font-black orbitron text-gray-400 uppercase tracking-widest flex items-center gap-2"><i data-lucide="zap" class="w-4 h-4 text-purple-400"></i> AI Trading Signals</h2>
                @if(!$isPremium)
                <span class="px-3 py-1 bg-amber-500/10 border border-amber-500/30 text-amber-400 text-[9px] font-bold orbitron uppercase rounded-full">Free Preview</span>
                @else
                <span class="px-3 py-1 bg-purple-500/10 border border-purple-500/30 text-purple-400 text-[9px] font-bold orbitron uppercase rounded-full flex items-center gap-1"><i data-lucide="crown" class="w-3 h-3"></i> Premium</span>
                @endif
            </div>

            {{-- Strategy Filter Tabs --}}
            <div class="flex flex-wrap gap-2 mb-2">
                <button class="tab-btn active orbitron" onclick="filterSignals('all', this)">All Signals</button>
                <button class="tab-btn orbitron" onclick="filterSignals('Swing', this)">Swing</button>
                <button class="tab-btn orbitron" onclick="filterSignals('Intraday', this)">Intraday</button>
                <button class="tab-btn orbitron" onclick="filterSignals('Long Term', this)">Long Term</button>
            </div>
            ...
            <div class="signal-card {{ $isLocked ? 'premium-locked' : '' }}" data-type="{{ $tip->trade_type }}">
                ...
                <div class="absolute inset-0 z-20 bg-black/60 backdrop-blur-sm rounded-[1rem] flex flex-col items-center justify-center gap-3">
                    <div class="w-10 h-10 bg-amber-500/20 rounded-full flex items-center justify-center"><i data-lucide="lock" class="w-5 h-5 text-amber-400"></i></div>
                    <p class="text-gray-400 text-xs font-bold orbitron uppercase tracking-widest">Premium Signal</p>
                    <a href="{{ route('pricing') }}" class="px-6 py-2 bg-amber-500 text-black font-black text-[10px] orbitron uppercase rounded-lg hover:scale-105 transition-all">Unlock Now</a>
                </div>
                @endif
                <div class="{{ $isLocked ? 'blur-content' : '' }}">
                    <div class="flex items-center justify-between mb-3">
                        <div class="flex items-center gap-2">
                            <h3 class="text-lg font-black orbitron text-white uppercase tracking-tight">{{ $tip->stock_name }}</h3>
                            <span class="px-2 py-0.5 bg-white/5 border border-white/10 rounded text-[8px] font-bold text-gray-400 uppercase orbitron">{{ $tip->trade_type }}</span>
                    ...
                    <div class="grid grid-cols-5 gap-2 mb-3">
                        ...
                        <div class="bg-emerald-500/[0.03] rounded-lg p-2.5 text-center border border-emerald-500/10"><div class="text-[7px] font-bold text-emerald-500/60 uppercase mb-0.5">Target 3</div><div class="text-sm font-black text-emerald-400">₹{{ number_format($tip->target_3, 1) }}</div></div>
                        <div class="bg-rose-500/[0.03] rounded-lg p-2.5 text-center border border-rose-500/10"><div class="text-[7px] font-bold text-rose-500/60 uppercase mb-0.5">Stop Loss</div><div class="text-sm font-black text-rose-400">₹{{ number_format($tip->stop_loss, 1) }}</div></div>
                    </div>
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-4">
                            <div class="flex items-center gap-1"><i data-lucide="shield" class="w-3 h-3 text-gray-500"></i><span class="text-[9px] font-bold text-gray-400 uppercase">Risk: <span class="{{ $tip->risk_level == 'Low' ? 'text-emerald-400' : 'text-amber-400' }}">{{ $tip->risk_level }}</span></span></div>
            ...
            @empty
            <div class="widget text-center py-12"><i data-lucide="radio" class="w-8 h-8 text-gray-700 mx-auto mb-3"></i><h3 class="orbitron font-bold text-gray-500 uppercase text-sm mb-2">No Active Signals</h3><p class="text-gray-600 text-xs">AI engine is scanning markets.</p></div>
            @endforelse

            @if(!$isPremium)
            <div class="widget bg-gradient-to-r from-purple-900/20 to-indigo-900/20 border-purple-500/20 text-center py-6">
                <div class="w-10 h-10 bg-amber-500/20 rounded-full flex items-center justify-center mx-auto mb-3"><i data-lucide="crown" class="w-5 h-5 text-amber-400"></i></div>
            ...
            <div class="widget bg-gradient-to-br from-purple-900/10 to-indigo-900/10 border-purple-500/10">
                <div class="widget-title orbitron flex items-center gap-2"><i data-lucide="cpu" class="w-3.5 h-3.5 text-purple-400"></i> How AI Signals Work</div>
                <div class="space-y-3">
            ...
            <div class="widget">
                <div class="widget-title orbitron flex items-center gap-2"><i data-lucide="bar-chart-2" class="w-3.5 h-3.5 text-purple-400"></i> Today's Market Summary</div>
        ...
        <aside class="space-y-4">
            ...
            <div class="widget">
                <div class="flex justify-between items-center mb-2">
                    <div class="widget-title orbitron mb-0">Market Pulse</div>
                ...
                <div class="grid grid-cols-2 gap-2 mt-2">
                    <div class="text-center p-1.5 bg-emerald-500/5 rounded-lg"><div class="text-[7px] font-bold text-gray-500 uppercase">Buyers</div><div class="text-sm font-black text-emerald-400 orbitron" id="buyVol">64.2%</div></div>
                    <div class="text-center p-1.5 bg-rose-500/5 rounded-lg"><div class="text-[7px] font-bold text-gray-500 uppercase">Sellers</div><div class="text-sm font-black text-rose-400 orbitron" id="sellVol">35.8%</div></div>
                </div>
            </div>

            {{-- Watchlist --}}
            <div class="widget">
                <div class="widget-title orbitron flex items-center gap-2"><i data-lucide="list" class="w-3.5 h-3.5 text-purple-400"></i> Watchlist</div>
                <div class="space-y-0">
                    ...
                    <div class="flex items-center justify-between py-1.5 border-b border-white/[0.02]">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-[10px] font-black orbitron {{ $ws['up'] ? 'text-emerald-400' : 'text-rose-400' }}">{{ $ws['change'] }}</span>
                        </div>
                    </div>
                    @endforeach
                </div>
            ...
            {{-- Performance --}}
            <div class="widget">
                <div class="widget-title orbitron">Performance</div>
                @php $bars = [
                ...
                @foreach($bars as $b)
                <div class="mb-2.5">
                    <div class="flex justify-between mb-0.5"><span class="text-[9px] font-bold text-gray-400 uppercase">{{ $b['l'] }}</span><span class="text-xs font-black text-white orbitron">{{ $b['v'] }}</span></div>
            ...
            @if($isPremium)
            <div class="widget">
                <div class="widget-title orbitron flex items-center gap-2"><i data-lucide="calculator" class="w-3.5 h-3.5 text-purple-400"></i> P&L Calculator</div>
                <div class="space-y-2">
                    ...
                    <button onclick="calculatePL()" class="w-full py-2 bg-purple-600 hover:bg-purple-700 text-white font-bold text-xs orbitron uppercase rounded-lg transition-all">Calculate</button>
                    <div id="plResult" class="text-center p-2 bg-white/5 rounded-lg hidden"><div class="text-[8px] font-bold text-gray-500 uppercase mb-0.5">Profit / Loss</div><div class="text-xl font-black orbitron" id="plValue">₹0</div></div>
                </div>
            </div>
            @else
            <div class="widget relative overflow-hidden">
                <div class="absolute inset-0 bg-black/50 backdrop-blur-sm z-10 flex flex-col items-center justify-center rounded-[1rem]"><i data-lucide="lock" class="w-5 h-5 text-amber-400 mb-1"></i><span class="text-[9px] font-bold text-gray-400 orbitron uppercase">Premium Feature</span></div>
                <div class="widget-title orbitron">P&L Calculator</div>
    ...
    {{-- ══════════════════════════════════════
         ...
         ══════════════════════════════════════ --}}
    <div class="mt-8 mb-5">
        <h2 class="text-sm font-black orbitron text-gray-400 uppercase tracking-widest mb-4 flex items-center gap-2"><i data-lucide="newspaper" class="w-4 h-4 text-purple-400"></i> Market News & Insights</h2>
        <div class="grid grid-cols-1 md:grid-cols-5 gap-3">
            @foreach($news as $n)
            <div class="widget hover:border-purple-500/20 transition-all cursor-pointer">
                <span class="inline-block px-2 py-0.5 bg-purple-500/10 text-purple-400 text-[8px] font-bold orbitron uppercase rounded mb-2">{{ $n['tag'] }}</span>
    ...
    <div class="mb-5">
        ...
        <div class="widget relative overflow-hidden text-center py-8">
            <div class="absolute inset-0 bg-black/40 backdrop-blur-sm z-10 flex flex-col items-center justify-center rounded-[1rem]">
                <i data-lucide="lock" class="w-6 h-6 text-amber-400 mb-2"></i>
        ...
        <div class="widget md:col-span-2">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-2">
                ...
                <div class="p-3 bg-white/[0.02] rounded-lg border border-white/[0.03]">
                    ...
                    <div class="text-sm font-black text-white orbitron mb-0.5">{{ $ind['value'] }}</div>
            ...
            <div class="mt-3 flex items-center gap-2">
                ...
                <span class="px-2 py-0.5 bg-emerald-500/10 text-emerald-400 text-[9px] font-bold orbitron uppercase rounded">Bullish (5/8)</span>
                <span class="px-2 py-0.5 bg-rose-500/10 text-rose-400 text-[9px] font-bold orbitron uppercase rounded">Bearish (2/8)</span>
                <span class="px-2 py-0.5 bg-amber-500/10 text-amber-400 text-[9px] font-bold orbitron uppercase rounded">Neutral (1/8)</span>
            </div>
        </div>

        {{-- Fear & Greed Gauge --}}
        <div class="widget text-center">
            <div class="widget-title orbitron flex items-center justify-center gap-2"><i data-lucide="heart-pulse" class="w-3.5 h-3.5 text-purple-400"></i> Fear & Greed Index</div>
            ...
            <div class="text-3xl font-black orbitron text-white mb-1">{{ $sentiment['value'] }}</div>
            <div class="text-xs font-bold uppercase orbitron {{ $sentiment['value'] > 60 ? 'text-emerald-400' : ($sentiment['value'] > 40 ? 'text-amber-400' : 'text-rose-400') }}">{{ $sentiment['label'] }}</div>
...
495:     @endif --}}
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
        ...
        @if($isPremium)
        <div class="widget">
            <div class="widget-title orbitron flex items-center gap-2"><i data-lucide="layers" class="w-3.5 h-3.5 text-purple-400"></i> NIFTY Options Chain</div>
            <div class="grid grid-cols-3 gap-2 mb-3">
                <div class="text-center p-2 bg-white/[0.02] rounded-lg"><div class="text-[7px] font-bold text-gray-500 uppercase">PCR</div><div class="text-sm font-black text-white orbitron">{{ $options['pcr'] }}</div></div>
                <div class="text-center p-2 bg-white/[0.02] rounded-lg"><div class="text-[7px] font-bold text-gray-500 uppercase">Max Pain</div><div class="text-sm font-black text-amber-400 orbitron">{{ $options['max_pain'] }}</div></div>
                <div class="text-center p-2 bg-white/[0.02] rounded-lg"><div class="text-[7px] font-bold text-gray-500 uppercase">IV</div><div class="text-sm font-black text-purple-400 orbitron">{{ $options['iv'] }}%</div></div>
            </div>
            <div class="grid grid-cols-2 gap-2 mb-3">
                <div class="text-center p-2 bg-emerald-500/5 rounded-lg"><div class="text-[7px] font-bold text-gray-500 uppercase">Total Call OI</div><div class="text-xs font-black text-emerald-400 orbitron">{{ $options['calls_oi'] }}</div></div>
        ...
        <div class="widget relative overflow-hidden">
            <div class="absolute inset-0 bg-black/40 backdrop-blur-sm z-10 flex flex-col items-center justify-center rounded-[1rem]"><i data-lucide="lock" class="w-6 h-6 text-amber-400 mb-2"></i><span class="text-xs font-bold text-gray-400 orbitron uppercase mb-2">Premium Feature</span><a href="{{ route('pricing') }}" class="px-4 py-1.5 bg-amber-500 text-black font-black text-[9px] orbitron uppercase rounded-lg">Upgrade</a></div>
            <div class="widget-title orbitron">Options Chain</div>
        ...
        {{-- Market Calendar --}}
        <div class="widget">
            <div class="widget-title orbitron flex items-center gap-2"><i data-lucide="calendar" class="w-3.5 h-3.5 text-purple-400"></i> Upcoming Events</div>
            <div class="space-y-0">
        ...
        <div class="widget">
            ...
            <div id="rrResult" class="hidden">
                <div class="grid grid-cols-3 gap-2 mb-3">
                    ...
                    <div class="text-center p-2 bg-rose-500/5 rounded-lg"><div class="text-[7px] font-bold text-gray-500 uppercase">Risk</div><div class="text-sm font-black text-rose-400 orbitron" id="rrRisk">0</div></div>
                    <div class="text-center p-2 bg-purple-500/5 rounded-lg"><div class="text-[7px] font-bold text-gray-500 uppercase">R:R Ratio</div><div class="text-sm font-black text-purple-400 orbitron" id="rrRatio">0</div></div>
                </div>
                ...
                <div class="text-center mt-2"><span class="text-[9px] font-bold orbitron uppercase" id="rrVerdict"></span></div>
            </div>
        </div>

        {{-- Trading Tips --}}
        <div class="widget">
            <div class="widget-title orbitron flex items-center gap-2"><i data-lucide="lightbulb" class="w-3.5 h-3.5 text-amber-400"></i> Pro Trading Tips</div>
            <div class="space-y-3">
    ...
    <div class="flex gap-3 mb-5">
        <button class="flex-1 py-3 bg-white/5 border border-white/5 rounded-xl flex items-center justify-center gap-2 text-[9px] font-bold text-gray-500 hover:text-white transition-all orbitron uppercase"><i data-lucide="download" class="w-3.5 h-3.5"></i> Download PDF Report</button>
        <button class="flex-1 py-3 bg-white/5 border border-white/5 rounded-xl flex items-center justify-center gap-2 text-[9px] font-bold text-gray-500 hover:text-white transition-all orbitron uppercase"><i data-lucide="file-spreadsheet" class="w-3.5 h-3.5"></i> Export CSV Data</button>
        <button class="flex-1 py-3 bg-white/5 border border-white/5 rounded-xl flex items-center justify-center gap-2 text-[9px] font-bold text-gray-500 hover:text-white transition-all orbitron uppercase"><i data-lucide="bell-ring" class="w-3.5 h-3.5"></i> Set Price Alert</button>
        <button class="flex-1 py-3 bg-white/5 border border-white/5 rounded-xl flex items-center justify-center gap-2 text-[9px] font-bold text-gray-500 hover:text-white transition-all orbitron uppercase"><i data-lucide="printer" class="w-3.5 h-3.5"></i> Print Terminal</button>
    </div>
    @endif
</div>
@endsection
...
<script>
    ...
    function calculatePL() {
        ...
        if(el&&val) { el.classList.remove('hidden'); val.innerText = (pl>=0?'+':'')+'₹'+pl.toLocaleString('en-IN',{minimumFractionDigits:2}); val.className = 'text-xl font-black orbitron '+(pl>=0?'text-emerald-400':'text-rose-400'); }
    }

    // ── AI Activity ──
    const actLog = document.getElementById('activityLog');
    if (actLog) {
        const msgs = ["Scanning derivatives for momentum...","Fibonacci validated on Sector-3","Volume anomaly detected...","AI confidence recalibrated to 98.1%","Bullish divergence confirmed","Stop-loss optimization running..."];
        setInterval(() => {
            const d = document.createElement('div'); d.className = 'flex items-start gap-2 py-1 border-b border-white/[0.02]';
            d.innerHTML = `<span class="text-[9px] font-mono text-purple-400/60 flex-shrink-0">${new Date().toLocaleTimeString()}</span><span class="text-[10px] text-gray-400 leading-snug">${msgs[Math.floor(Math.random()*msgs.length)]}</span>`;
            actLog.prepend(d); if(actLog.children.length > 15) actLog.removeChild(actLog.lastChild);
        }, 5000);
    }
    ...
    function calcRR() {
        ...
        if(ratio >= 2) { v.innerText = '✅ Excellent Trade Setup'; v.className = 'text-[9px] font-bold orbitron uppercase text-emerald-400'; }
        else if(ratio >= 1) { v.innerText = '⚠️ Acceptable Risk'; v.className = 'text-[9px] font-bold orbitron uppercase text-amber-400'; }
        else { v.innerText = '❌ Poor Risk-Reward'; v.className = 'text-[9px] font-bold orbitron uppercase text-rose-400'; }
    }
