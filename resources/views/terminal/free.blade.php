    ...
    <div class="container mx-auto max-w-6xl relative z-10">
        ...
        <div class="glass-panel border border-white/5 overflow-hidden mb-20 fade-in-up" style="animation-delay: 0.2s; border-radius: 4px;">
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse bg-[#0c0c0e]">
                    <thead>
                        <tr class="bg-[#1f1635] text-[10px] font-bold orbitron text-white uppercase tracking-wider border-b border-purple-500/30">
                            <th class="px-6 py-3 min-w-[120px]">STOCK</th>
                            <th class="px-6 py-3 text-center">TYPE</th>
                            <th class="px-6 py-3">ENTRY</th>
                            <th class="px-6 py-3">SL</th>
                            <th class="px-6 py-3">T1</th>
                            <th class="px-6 py-3">T2</th>
                            <th class="px-6 py-3 text-center">AI %</th>
                            <th class="px-6 py-3">TIME</th>
                            <th class="px-6 py-3">STATUS</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-white/5">
                        @forelse($db_signals as $s)
                            <tr class="hover:bg-white/[0.04] transition-all border-b border-white/5 h-20 group">
                                <td class="px-6 py-4">
                                    <div class="flex flex-col">
                                        <span class="font-black orbitron text-white text-sm tracking-tighter">{{ $s->stock_symbol }}</span>
                                        <span class="text-[9px] font-bold text-slate-500 orbitron uppercase tracking-widest mt-1">Equity</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <span class="text-[10px] font-black orbitron px-3 py-1 rounded-lg bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 tracking-widest">
                                        BUY
                                    </span>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex flex-col">
                                        <span class="text-[9px] font-bold text-slate-500 orbitron uppercase mb-1">Entry</span>
                                        <span class="text-sm font-bold text-white orbitron">₹{{ number_format($s->entry_price, 2) }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex flex-col">
                                        <span class="text-[9px] font-bold text-rose-500/50 orbitron uppercase mb-1">Stop Loss</span>
                                        <span class="text-xs font-bold text-rose-400 orbitron">₹{{ number_format($s->stop_loss, 2) }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex flex-col text-purple-400">
                                        <span class="text-[9px] font-bold opacity-50 orbitron uppercase mb-1">Target 1</span>
                                        <span class="text-sm font-bold orbitron">₹{{ number_format($s->target_1, 2) }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex flex-col text-purple-400">
                                        <span class="text-[9px] font-bold opacity-50 orbitron uppercase mb-1">Target 2</span>
                                        <span class="text-sm font-bold orbitron">₹{{ number_format($s->target_2, 2) }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <div class="inline-flex flex-col items-center">
                                        <span class="text-lg font-black text-purple-400 orbitron leading-none">{{ $s->confidence_level }}%</span>
                                        <span class="text-[8px] font-bold text-slate-600 orbitron uppercase mt-1">Accuracy</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex flex-col">
                                        <span class="text-[9px] font-bold text-slate-500 orbitron uppercase mb-1">Detected</span>
                                        <span class="text-xs font-bold text-white opacity-80 orbitron">{{ $s->created_at->format('H:i') }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-2 group-hover:scale-105 transition-transform">
                                        ...
                                        <span class="text-[10px] font-black orbitron tracking-[0.2em] text-emerald-500 uppercase">Live</span>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr><td colspan="9" class="px-6 py-20 text-center text-gray-600 orbitron text-xs font-bold uppercase tracking-[0.3em] italic">No free algorithmic signals detected in the current session.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
